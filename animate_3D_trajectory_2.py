...
linewidth_3d = 2
dyn_agent_lines = sum([ax.plot([], [], [], 'r', linewidth=linewidth_3d) for dat in dyn_agents], [])
emd_agent_lines = sum([ax.plot([], [], [], 'r', linestyle='--', linewidth=linewidth_3d) for dat in emd_agents], [])
target_lines = sum([ax.plot([], [], [], 'bo') for dat in targets], [])

dyn_agent_lines[0].label = 'Agent Path (Dyn)'
...
line_ani = animation.FuncAnimation(fig, animate, init_func=init, frames=500, interval=10,
        blit=True, repeat=True)

# line_ani.save('test.gif', writer='imagemagick', fps=30)
